...
EVE_VERSION=0.62.0 WORKFLOW_WORLD_E2E_URL=postgres://user:pass@127.0.0.1:5432/postgres pnpm run test:e2e
```
...
package pins the newest line's verified release (0.62.0). A minor does not reliably
identify a `@workflow/*` set, so the entries name exact patches. The current
window, {0.58.x, 0.62.x} (0.59, 0.60 and 0.61 skipped), carries two sets: world
beta.35, world-local beta.44 and core beta.51 on 0.58, and world beta.36,
world-local beta.45 and core beta.53 from 0.61.0 on. The execution model is the
same on both lines -- turns run inside the session's own run -- so the enabled
entries cover each set once (0.58.1 and 0.62.0);
