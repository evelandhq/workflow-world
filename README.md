...
`eve@0.58.1` pin and the `@workflow/*` set `@workflow/world-testing@beta.51`
...
```bash
# by hand, not in CI: one dispatch held open for 330s (past undici's 300s fetch deadline) against the production
...
current supported window, {0.55.x, 0.58.x}, carries one set: world beta.35,
world-local beta.44 and core beta.51, unchanged since the 0.54.4 patch (0.57.0
and 0.58.1 re-bundled it without moving a version). The pins here follow the
newest line;
